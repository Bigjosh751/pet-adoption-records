...
export function PetAdaptorHistoryTable({ number }: { number?: number }) {
  ...
  const displayedAdoptors = number ? adaptors.slice(0, number) : adaptors;
  return displayedAdoptors.length > 0 ? (
    <Table className="myfont">
      ...
      <TableBody>
        {displayedAdoptors.map((adaptor: any, index: number) => (
          <TableRow
            key={index}
